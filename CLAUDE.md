...
deriveLots(transactions) → Lot[]
  ...
  - sell 匹配优先级：lotId 精确 → gridExecutionId → 成本升序（非按时间 FIFO），统一在 matchSellAgainstLots()
...
| `frontend/src/services/favoriteService.ts` | Favorite fund helper (`addFavoriteFund`) |
